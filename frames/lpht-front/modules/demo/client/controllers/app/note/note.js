'use strict';
angular.module('demo').controller('NoteCtrl', ['$scope', '$http', function($scope, $http) {
  $http.get('api/app/note/notes.json').then(function(resp) {
  ...
  $scope.deleteNote = function(note) {
    ...
    if (note.selected) {
      $scope.note = $scope.notes[0];
      if ($scope.notes.length) { $scope.notes[0].selected = true ;}
    }
  };
