 ...
 * @author jxk
 */
angular.module('lpht.germplasm').controller('BreedingRemindCtrl', ['$scope', '$http', '$uibModal', 'uiGridConstants', 'toaster', function($scope, $http, $uibModal, uiGridConstants, toaster) {
  ...
  gm.gridOptions = {
    ...
    columnDefs: [
      ...
      { field: 'firClassStr', displayName: '一级分类' },
      { field: 'secClassStr', displayName: '二级分类' },
      { field: 'gpValueStr', displayName: '材料价值' },
      { field: 'signType', displayName: '关系运算符' },
      { field: 'remindNumber', displayName: '繁种警戒数量' },
      { field: 'unitStr', displayName: '单位' },
  ...
  gm.openModal = function(type) {
    var modalInstance = $uibModal.open({
      templateUrl: 'breedingRemindModal',
      controller: 'ModalBreedingRemindCtrl',
      size: 'lg',
      backdrop: 'static',
      resolve: {
        modalType: function() {
  ...
  function getGrid() {
    ...
    $http.post('api/germmanage/breedingRemind/queryBreedingRemind', {})
 ...
 * @author jxk
 */
angular.module('lpht.germplasm').controller('ModalBreedingRemindCtrl', ['$scope', '$http', '$uibModalInstance', 'toaster', 'modalType', 'rowEntity', function($scope, $http, $uibModalInstance, toaster, modalType, rowEntity) {
  ...
  fm.ok = function() {
    //保存数据
    var params = { remindId: fm.rowEntity.remindId, gpValue: fm.rowEntity.gpValue, remindNumber: fm.rowEntity.remindNumber, unit: fm.rowEntity.unit, remark: fm.rowEntity.remark };
    $http.post('api/germmanage/breedingRemind/updateBreedingRemind', params)
      .success(function(data) {
        if (data.retcode) {
  ...
  //加载单位信息
  $http.get('api/security/helptypecode/typecodes/materialValue,unitType')
    .success(function(data) {
      fm.materialData = data.materialValue;
      fm.unitData = data.unitType;
    })
    .error(function(ex) {
      toaster.pop({ type: 'error', title: ex, toasterId: 1 });
    });
